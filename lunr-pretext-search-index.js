...
},
{
  ...
  "body": " Defining Functions  Sage comes with many built-in functions. Math terminology is not always standard, so be sure to read the documentation to learn what these functions do and how to use them. In Sage, you can also define your own functions. Defining a function in Sage is similar to defining a function in Python.  To define a function in Sage, use the def keyword followed by the function name and the function's arguments. The function's body is indented. The return keyword is used to return a value from the function.  Here we have a function that takes a name as an argument and returns a greeting:   Notes  For an extra challenge, you can implement our greetings function more elegantly with string formatting.    Try calling the function for yourself. Be sure to run the Sage cell with the function definition first to define the function and use it in other cells. When you run the cell that defines the function, you won't see any output. You will only see output when you call the function.   You can also add input validation to your functions to make them more robust.   Try calling the function with an empty string or a non-string argument to see the input validation in action.    Functions can also include a docstring to provide documentation for the function. The docstring is a string that appears as the first statement in the function body. It is used to describe what the function does and how to use it.   Let's use what we learned about functions to define a function that is relevant to mathematics. You may have heard of Pascal's Triangle, a triangular array of numbers in which each number is the sum of the two numbers directly above it. Here is a function that generates the (0-indexed)row of Pascal's Triangle:   After running the above cell, you can call the function printPascalRow .   "
},
{
  "id": "documentation",
  "level": "1",
...
},
{
  ...
  "body": " Operations on Sets    Union of Sets  There are two distinct methods available in Sage for calculating unions.  Suppose and . We can use the union() function to calculate .   Notes  This operation can be visualized in real-world scenarios such as merging two distinct music playlists into one. In this case, any song that appears in both playlists will only be listed once in the merged playlist.    Alternatively, we can also use the | operator to perform the union operation.      Intersection of Sets  Similar to union, there are two methods of using the intersection function in Sage.  Suppose and . We can use the intersection() function to calculate .   Notes  A practical application of set intersection could be finding common members between two different social media groups. Members who belong to both groups represent the intersection of these groups.    Alternatively, we can also use the & operator to perform the intersection operation.      Difference of Sets  We have two methods to solve for difference as well.  Suppose and . We can use the difference() function to calculate the difference between 2 sets.   Notes  In a real-life context, this can be seen as identifying items on a restaurant menu (set A) that you have not yet tried (set B), with A - B representing the new dishes to explore.    Alternatively, we can also use the - operator to perform the difference operation.      Multiple Sets  For doing operations involving multiple sets, we can repeat the operations to get our results, let us see an example.  Suppose , and . To find the union of all three sets, we will repeat the union() function.   Alternatively, we can also repeat the | operator to perform the union operation.   Similar operations can also be done using the intersection() and difference() functions as well as their operators.     Complement of Sets  Let be the universal set. Given the set . We can use the difference() function to find the complement of .   Notes  This is analogous to having a list of all possible ice cream flavors (U) and identifying those flavors you have yet to try (A').    Alternatively, we can also use the - operator as well.      Cartesian Product of Sets  Suppose and . We can use the cartesian_product() and Set() functions to display the Cartesian product .   Notes  This concept can be applied in situations like determining all possible combinations of two different sets, such as shirt colors and pants styles in a wardrobe.    Alternatively, we can use the . notation to find the Cartesian product.      Power Set of Sets  The power set of a set is the set of all subsets of , including the empty set and itself. This Sage code defines a set and uses the powerset() function to generate all subsets of . The list() function is then used to convert the power set iterator into a list.   Notes  Exploring the power set of a set can be particularly useful in fields such as combinatorics, probability theory, and computer science, where understanding the relationships between subsets of a given set is crucial.     "
},
{
...
},
{
  ...
  "body": " Introduction to Relations  A relation  from set to set is any subset of the Cartesian product , indicating that . We can ask Sage to decide if is a relation from to . First construct the Cartesian product . Then build the set of all subsets of . Finally ask if is a subset of .   Notes  In database management, relations model the connections between different entities (e.g., customers and orders). Each record in one table can relate to records in another, facilitating complex queries and data analytics essential for operational intelligence and decision making.   Recall, the Cartesian product consists of all possible ordered pairs , where and . Each pair combines an element from set with an element from set .  In this example, an element in the set relates to an element in if the element from is twice the element from .   Let's use relations to explore matching items of clothes. Let's define two sets, jackets and shirts, as examples:   Notes  In online shopping platforms, relations help in personalizing customer experiences. Products and user preferences can be modeled as sets, with relations based on past purchases or browsing history to recommend new products effectively.       The Cartesian product of jackets and shirts includes all possible combinations of jackets with shirts.    Notes  Recommendation systems in services like streaming and e-commerce use relations to connect users to products or media they're likely to enjoy, based on similar user preferences and behaviors, enhancing user engagement and satisfaction.   Since the Cartesian product returns all the possible combinations, some jackets and shirts will clash. Let's create a relation from jackets to shirts based on matching the items of clothing.   "
},
{
...
},
{
  ...
  "number": "5.3",
  "title": "Digraphs",
  "body": " Digraphs  A digraph, or directed graph, is a visual representation of a relation on the set . Every element in set is shown as a node (vertex). An arrow from the node to the node represents the pair on the relation .   Notes  Digraphs are essential in the modeling of computer networks where nodes represent computers or routers and directed edges represent the direction of data flow. This aids in understanding and optimizing network traffic and routing protocols.    We can add a title to the digraph with the name parameter.   Real-world Application: Social Network Analysis  In social network analysis, digraphs are used to represent relationships where direction matters, such as Twitter followers or citations in academic papers. This helps in identifying influential users or key research papers.    If the digraph does not contain a relation from a node to itself, we can omit the loops=true parameter. If we happen to forgot to include the parameter when we need to, Sage will give us a descriptive error message.    Real-world Application: Systems Biology  Directed graphs are crucial in systems biology to model regulatory networks where nodes represent genes or proteins, and edges represent regulatory influences, helping to decode complex biological systems and interactions.   We can also define the digraph using pair notion for relations.   Alternatively, we can define the digraph directly. The element on the left of the : is a node. The node relates to the elements in the list on the right of the : .   "
},
{
  "id": "properties",
  "level": "1",
...
},
{
  "id": "equivalence-20",
  "level": "2",
  "url": "equivalence.html#equivalence-20",
  "type": "Paragraph (with a defined term)",
  "number": "",
  "title": "",
  "body": "Transitive "
...
},
{
  ...
  "number": "7.1",
  "title": "Basics",
  "body": " Basics   Graph Definition  In discrete mathematics, a graph is a fundamental concept used to model pairwise relations between objects. A graph is defined as an ordered pair , where:    is a set of vertices (also called nodes).    is a set of edges (also called links or arcs), which are unordered pairs of vertices (in an undirected graph) or ordered pairs of vertices (in a directed graph).    The vertices represent the objects, and the edges represent the connections or relationships between these objects.  We can define a graph in Sage by listing the vertices and edges:    Notes  Graph theory is pivotal in areas such as network routing and social networking. For example, in telecommunications, graphs are used to model and optimize routing of communications like telephone traffic or internet data packets through a network of nodes and links, ensuring efficient and fault-tolerant network design.   Another way would be specifying the edges only, where each edge is defined by a pair of vertices:   Both methods allow for the creation and visualization of graphs in Sage, with the choice of method depending on the format of the data available and personal preference for clarity and organization.  Graphs can also be defined by specifying the neighbors of each vertex. In Sage, an undirected graph can be created using a dictionary to represent the adjacency list of each vertex, indicating the vertices that are connected by an edge.   Sage offers a wide array of predefined graphs for experimentation and study. These graphs represent important concepts and structures in graph theory.  Examples of creating and visualizing some well-known predefined graphs:   These examples include the Petersen graph, a complete graph on 5 vertices (also known as the K5 graph), the tetrahedral graph, the dodecahedral graph, and the hexahedral (or cubic) graph, showcasing a variety of graph structures.  Defining a graph with data structures before creation allows for managing complex graphs efficiently. This approach is suitable for graphs with a large number of vertices or intricate connectivity.  An example using predefined data to define a graph:   This method provides a clear and organized way to define the relationships between vertices, especially for complex or large-scale graphs.    Weighted Graphs  Weighted graphs extend the concept of graphs by associating a weight, typically a numerical value, with each edge. These weights can represent distances, costs, capacities, or other quantities relevant to the problem being modeled. In Sage, weighted graphs are easily handled, allowing for the exploration of algorithms and properties specific to these types of graphs.  To create a weighted graph, you can add a third element to each pair of vertices representing the weight.  Here is an example of defining a weighted graph with five vertices, forming a pentagon where each edge has a unique weight:   In the code above, the graph G is defined with vertices and edges, where each edge is a tuple containing two vertices and the weight of the edge. The weighted=True parameter indicates that the graph is weighted, and the plot(edge_labels=True) method visualizes the graph with edge weights displayed.    Graph Characteristics  Understanding the characteristics of graphs is fundamental to analyzing their properties and behaviors. These characteristics provide essential insights into the structure of the graph. Sage offers various functions to compute and analyze these characteristics, which are crucial for graph theory studies.  We start by plotting the following graph.   To list all vertices of a graph, we use the G.vertices() method, which returns a list of the graph's vertices.   To list all edges of a graph, the G.edges() method is used. This returns a list of tuples representing the graph's edges.   To remove empty labels, include labels=false .   The order of a graph is the number of vertices .  To find the order of the graph:   The size of a graph is the number of edges .  To find the size of the graph:   The degree of a vertex in is the number of edges incident to the vertex.  To find the degree of vertex :   The method G.degree_sequence() provides a list of degrees of all vertices in the graph in decreasing order.     Graphs and Matrices  The adjacency matrix of a graph is a square matrix used to represent which vertices of the graph are adjacent to which other vertices. Each entry in the matrix is equal to 1 if there is an edge from vertex i to vertex j , and is equal to 0 otherwise.  To create a graph from an adjacency matrix:   In this example, the matrix A represents the adjacency matrix of a graph, and Graph(A) creates the graph based on that matrix. The plot shows the resulting graph.  The incidence matrix is another matrix representation of a graph, which describes the relationship between vertices and edges. In this matrix, rows correspond to vertices, and columns correspond to edges, with entries indicating whether a vertex is incident to an edge.  Besides creating graphs from matrices, you can also obtain these matrix representations from a given graph in Sage.  To get the adjacency matrix of a graph:   And to get the incidence matrix:     Manipulating Graphs in Sage  Modifying graphs by adding or removing vertices and edges allows us to observe how these changes affect graph properties. Sage provides user-friendly functions for these modifications, facilitating dynamic exploration of graphs.  To add a vertex to a graph:   To remove a vertex from a graph:   To add an edge between two vertices in a graph:   To delete an edge from a graph:   Observe that deleting a vertex that was not there, returns an error whilst deleting an edge that was not in the graph leaves the graph unchanged. Adding a vertex or edge that was already in the graph, leaves the graph unchanged.   You can also add a list of vertices or edges.    "
},
{
  "id": "sec-plot-options",
  "level": "1",
...
},
{
  ...
  "number": "7.3",
  "title": "Paths",
  "body": " Paths  A path between two vertices and is a sequence of consecutive edges starting at and ending at .  To get all paths between two vertices:   The length of a path is defined as the number of edges that make up the path.  Finding the shortest path between two vertices can be achieved using the shortest_path() function:    Notes  Pathfinding in graphs is a key algorithm in logistics to determine the most efficient route between locations. It's used in GPS navigation systems to find the quickest route for vehicles, and in network design to optimize data flow between servers, enhancing the efficiency of network infrastructures.   A graph is said to be connected if there is a path between any two vertices in the graph.  To determine if a graph is connected, we can use the is_connected() function:   A connected component of a graph is a maximal connected subgraph of . If the graph is connected, then it has only one connected component.  For example, the following graph is not connected:   To identify all connected components of a graph, the connected_components() function can be utilized:   We can visualize the graph as a disjoint union of its connected components, by plotting it.   The diameter of a graph is the length of the longest shortest path between any two vertices.   A graph is bipartite if its set of vertices can be divided into two disjoint sets such that every edge connects a vertex in one set to a vertex in the other set:   "
},
{
  "id": "isomorphism",
  "level": "1",
  ...
  "number": "7.4",
  "title": "Isomorphism",
  "body": " Isomorphism  Informally, we can say that an isomorphism is a relation of sameness between graphs. Let's say that the graphs and are isomorphic if there exists a bijection such that .  This means there is a bijection between the set of vertices such that every time two vertices determine an edge in the first graph, the image of these vertices by the bijection also determines an edge in the second graph, and vice versa. Essentially, the two graphs have the same structure, but the vertices are labeled differently.   Notes  In chemistry, isomorphism is crucial in the study of molecules. Chemical isomers have the same molecular formula but different structures (or graphs), affecting their chemical properties and reactions. Understanding graph isomorphism helps chemists identify and differentiate these molecules efficiently.    The sage is_isomorphic() method can be used to check if two graphs are isomorphic. The method returns True if the graphs are isomorphic and False if the graphs are not isomorphic.   The invariants under isomorphism are conditions that can be checked to determine if two graphs are not isomorphic. If one of these fails then the graphs are not isomorphic. If all of these are true then the graph may or may not be isomorphic. The three conditions for invariants under isomorphism are:      Notes  Graph isomorphism has implications in computer science, particularly in data mining and machine learning. Algorithms that detect isomorphisms can help identify similar data structures in databases, facilitating the classification and clustering of data based on structural similarity.   To summarize, if one graph is connected and the other is not, then the graphs are not isomorphic. If the number of vertices and edges are different, then the graphs are not isomorphic. If the degree sequences are different, then the graphs are not isomorphic. If all three invariants are satisfied, then the graphs may or may not be isomorphic.  Let's define a function to check if two graphs satisfy the invariants under isomorphism. Make sure you run the next cell to define the function before using the function.    Notes  Network theory uses graph isomorphism to analyze and understand different networks, such as social networks or communication networks. By identifying isomorphic subgraphs, researchers can detect patterns and anomalies across different network systems.    If we use invariants_under_isomorphism on the and , the output will let us know that the graphs may or may not be isomorphic. We can use the is_isomorphic() method to check if the graphs are definitively isomorphic.   "
},
{
  "id": "isomorphism-2",
  "level": "2",
...
},
{
  "id": "isomorphism-8",
  "level": "2",
  "url": "isomorphism.html#isomorphism-8",
  "type": "Paragraph (with a defined term)",
  "number": "",
  "title": "",
...
},
{
  ...
  "body": " Euler and Hamilton   Euler  An Euler path is a path that uses every edge of a graph exactly once. An Euler path that is a circuit is called an Euler circuit .  The idea of an Euler path emerged from the study of the Königsberg bridges problem. Leonhard Euler wanted to know if it was possible to walk through the city of Königsberg, crossing each of its seven bridges exactly once. This problem can be modeled as a graph, with the land masses as vertices and the bridges as edges.    Notes  Eulerian circuits are not only academic but have practical applications in logistics and waste management. For example, garbage collection routes can be planned as Eulerian circuits to minimize travel and costs. Similarly, postmen use Eulerian paths to optimize mail delivery routes.   While exploring this problem, Euler discovered the following:   A connected graph has an Euler circuit iff every vertex has an even degree.    A connected graph has an Euler path iff there are at most two vertices with an odd degree.   We say that a graph is Eulerian if contains an Euler circuit.  We can use Sage to determine if a graph is Eulerian.   Notes  In computer networking, designing networks to ensure resilience and fault tolerance often involves constructing Eulerian graphs, where a network can remain operational even if a connection fails.    Since this returns False , we know that the graph is not Eulerian. Therefore, it is not possible to walk through the city of Königsberg, crossing each of its seven bridges exactly once.  We can use path=True to determine if a graph contains an Euler path. Sage will return the beginning and the end of the path.   If the graph is Eulerian, we can ask Sage to find an Euler circuit with the eulerian_circuit function. Let's take a look at the following graph.   If we are not interested in the edge labels, we can set labels=False . We can also set return_vertices=True to get a list of vertices for the path     Hamilton  A Hamilton path is a path that uses every vertex of a graph exactly once. A Hamilton path that is a circuit is called a Hamilton circuit . If a graph contains a Hamilton circuit, we say that the graph is Hamiltonian .  Hamilton created the \"Around the World\" puzzle. The object of the puzzle was to start at a city and travel along the edges of the dodecahedron, visiting all of the other cities exactly once, and returning back to the starting city.  We can represent the dodecahedron as a graph and use Sage to determine if it is Hamiltonian. See for yourself if the dodecahedron is Hamiltonian.   Real-world Application  Hamiltonian circuits are integral to creating efficient routing algorithms used by services like Google Maps and UPS for finding the shortest path through multiple points, optimizing travel and delivery times.    We can ask Sage to determine if the dodecahedron is Hamiltonian.   By running Graph.is_hamiltonian?? we see that Sage uses the traveling_salesman_problem() function to determine if a graph is Hamiltonian.  The traveling salesperson problem is a classic optimization problem. Given a list of cities and the lengths between each pair of cities, what is the shortest possible route that visits each city and returns to the original city? This is one of the most difficult problems in computer science. It is NP-hard , meaning that no efficient algorithm is known to solve it. The complexity of the problem increases with the number of nodes. When working with many nodes, the algorithm can take a long time to run.  Let's explore the following graph:   Scientific Research  In scientific research, especially in genetics, Hamiltonian paths are used to solve the DNA sequencing problem by finding the shortest possible unique sequence that can represent a set of observed sequences.    We can ask Sage if the graph contains a Hamiltonian cycle.   The function hamiltonian_cycle returns True and lists an example of a Hamiltonian cycle as the list of vertices [1, 2, 3, 4, 5] . This is just one of the many Hamiltonian cycles that exist in the graph. Now lets find the minimum Hamiltonian cycle.   Now we have the plot of the minimum Hamiltonian cycle. The minimum Hamiltonian cycle is the shortest possible route that visits each city and returns to the original city. The minimum Hamiltonian cycle is the solution to the traveling salesperson problem. We can ask Sage for the sum of the weights of the edges in the minimum Hamiltonian cycle.   If there is no Hamiltonian cycle, Sage will return False . If we use the backtrack algorithm, Sage will return a list that represents the longest path found.    "
},
{
  "id": "subsec-Euler-2",
  "level": "2",
  "url": "euler-and-hamilton.html#subsec-Euler-2",
  "type": "Paragraph (with a defined term)",
  "number": "",
  "title": "",
...
},
{
  "id": "subsec-Euler-3",
  "level": "2",
  "url": "euler-and-hamilton.html#subsec-Euler-3",
  "type": "Paragraph (with a defined term)",
  "number": "",
  "title": "",
...
},
{
  "id": "subsec-Euler-6",
  "level": "2",
  "url": "euler-and-hamilton.html#subsec-Euler-6",
  "type": "Paragraph (with a defined term)",
  "number": "",
  "title": "",
...
{
  "id": "subsec-Hamilton-2",
  "level": "2",
  "url": "euler-and-hamilton.html#subsec-Hamilton-2",
  "type": "Paragraph (with a defined term)",
  "number": "",
  "title": "",
...
{
  "id": "subsec-Hamilton-10",
  "level": "2",
  "url": "euler-and-hamilton.html#subsec-Hamilton-10",
  "type": "Paragraph (with a defined term)",
  "number": "",
  "title": "",
  "body": "NP-hard "
...
},
{
  ...
  "body": " Tables of Operations   This section delves into the representation of meet and join operations within lattices using operation tables. Such tables are pivotal for visualizing the algebraic structure of lattices and understanding how elements combine under each operation.    Meet Operation Table  The meet operation table illustrates the greatest lower bound, or meet, for every pair of elements in the lattice.  For outputting the table as a matrix, we need to specify that the poset is indeed a lattice, thus requiring us to use the function LatticePoset() . Then we can use the function meet_matrix() to process the table.   From the output matrix, we can see that each entry is not the actual value of the meet of the elements and but just its position in the lattice. Sage does not have a specific function to output the exact meet, but we can define our own function for this purpose.   We can further improve our function to show the output as a table instead of a matrix.     Join Operation Table  Conversely, the join operation table presents the least upper bound, or join, for each pair of lattice elements.  Similarly, we can use the function join_matrix() to process the table.   We can also output the elements of the poset by slightly changing the function we previously defined.   We can also output the join table just as shown above for meet.    "
},
{
